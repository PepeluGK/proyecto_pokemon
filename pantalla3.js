...
let userPokemon = JSON.parse(getCookie("selectedPokemon"));
let rivalPokemon = JSON.parse(getCookie("rivalPokemon"));
userPokemon.max_hp = userPokemon.hp;
rivalPokemon.max_hp = rivalPokemon.hp;
const logs = document.getElementById('log');

//barras de vida
function updateStats() {
    document.getElementById("userName").textContent = userPokemon.name;
    document.getElementById("rivalName").textContent = rivalPokemon.name;

    document.getElementById("userImg").src = userPokemon.img;
    document.getElementById("rivalImg").src = rivalPokemon.img;

    const userHealthBar = document.getElementById("userHealthBar");
    const rivalHealthBar = document.getElementById("rivalHealthBar");

    const userHealthWidth = Math.min((userPokemon.hp / userPokemon.max_hp) * 100, 100);
    const rivalHealthWidth = Math.min((rivalPokemon.hp / rivalPokemon.max_hp) * 100, 100);

    userHealthBar.style.width = userHealthWidth + "%";
...
function playerAttack() {
    if (!((userPokemon.attack - rivalPokemon.defense) < 0)) {
        ...
        addLog(`${userPokemon.name} ha atacado a ${rivalPokemon.name}`);

        if (rivalPokemon.hp === 0) {
            alert("¡Ganaste! El rival ha quedado fuera de combate.");
            disableButtons();
            createBackButton();
        } else {
            setTimeout(rivalTurn, 1000);
        }
    }
    else {
        addLog(`${userPokemon.name} tiene tan poco ataque, que no hace ni media`);
        setTimeout(rivalTurn, 1000);
    }
}

//rival actúe aleatoriamente
function rivalTurn() {
...
function rivalAttack() {
    if (!((rivalPokemon.attack - userPokemon.defense) < 0)) {
        ...
        addLog(`${rivalPokemon.name} ha atacado a ${userPokemon.name}`);
    
        if (userPokemon.hp === 0) {
            alert("¡Perdiste! Tu Pokémon ha quedado fuera de combate.");
            disableButtons();
            createBackButton();
        }
    }
    else {
        addLog(`${rivalPokemon.name} tiene tan poco ataque, que no hace ni media`);
    }
...
function playerHeal() {
    userPokemon.hp += 20;
    if (userPokemon.hp > userPokemon.max_hp) userPokemon.hp = userPokemon.max_hp;
    updateStats();

    addLog(`${userPokemon.name} se ha curado`);
...
function rivalHeal() {
    rivalPokemon.hp += 20;
    if (rivalPokemon.hp > rivalPokemon.max_hp) rivalPokemon.hp = rivalPokemon.max_hp;
    updateStats();

    addLog(`${rivalPokemon.name} se ha curado`);
